defmodule SkepticBot.CheckPodcastEpisodes do
  ...
  defp check_audio_file_exists(%{exists: true, audio_url: audio_url})
  ...
  defp display_results(results, file) do
    ...
    issues =
      Enum.filter(results, fn r ->
        ...
        transcribing_issue =
          r.transcribing[:exists] != true || r.transcribing[:state] != "completed"

        embeddings_issue =
          r.transcribing[:exists] == true &&
    ...
    if length(issues) > 0 do
      ...
      Enum.each(issues, fn issue ->
        IO.puts(
          file,
          "  - Episode #{issue.id} (#{issue.external_id}): #{get_issue_description(issue)}"
        )
      end)
    else
  ...
  defp get_issue_description(episode) do
    cond do
      ...
      episode.transcribing[:exists] == true && episode.transcribing[:state] == "completed" &&
          episode.embeddings[:exists] != true ->
